...
beforeEach(() => {
  dir = mkdtempSync(path.join(tmpdir(), 'broll-cfg-'));
});
...
describe('loadConfig', () => {
  it('produces sensible defaults with no config file', () => {
    const config = loadConfig({ cwd: dir, env: { BROLL_HOME: path.join(dir, 'home') } as NodeJS.ProcessEnv });
  ...
  it('reads broll.config.json from cwd and merges defaults', () => {
    writeFileSync(
      path.join(dir, 'broll.config.json'),
    ...
    expect(config.configPath).toBe(path.join(dir, 'broll.config.json'));
  });

  it('rejects malformed config files loudly', () => {
    writeFileSync(path.join(dir, 'broll.config.json'), JSON.stringify({ brand: { watermarkOpacity: 5 } }));
